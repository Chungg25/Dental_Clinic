...
namespace Dental_Clinic.GUI.LeTan.ThanhToan
{
    public partial class FormChiTietHoaDon : Form
    {
        ...
        private bool thanhToanTienMat = false;
        ...
        private void vbXuatHoaDon_Click(object sender, EventArgs e)
        {
            ...
            // Không xuất hoá đơn nếu chưa thanh toán

            // Không xuất hoá đơn nếu chưa chọn phương thức thanh toán
            if (!thanhToanTienMat && !thanhToanThanhCong)
            {
                MessageBox.Show("Vui lòng chọn phương thức thanh toán trước khi xuất hoá đơn!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            // Không xuất hoá đơn nếu không chọn phương thức thanh toán
            if (thanhToanTienMat && tbTienKhachDua.Text == "")
            {
                MessageBox.Show("Vui lòng nhập số tiền khách đưa trước khi xuất hoá đơn!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            // Không xuất hoá đơn nếu số tiền khách đưa không đủ
            if (thanhToanTienMat && float.Parse(tbTienKhachDua.Text) < tongTien)
            {
                MessageBox.Show("Số tiền khách đưa không đủ để thanh toán hóa đơn!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            // Xuất hoá đơn
            // Nếu chọn chuyển khoản thì không cần nhập số tiền khách đưa và không cần tính tiền thừa trả lại
            if (thanhToanThanhCong)
            {
            ...
            {
                leTanBUS.XuatHoaDonPDF(benhNhan, dsHoaDon, float.Parse(tbTienKhachDua.Text));
            }
        }
    }
}
