...
namespace Dental_Clinic.DAO.LeTan
{
    public class LeTanDAO
    {
        ...
        public LuongDTO LayThongTinLuong(int maLeTan, DateTime thang, DateTime nam)
        {
            ...
            {
                cmd.Parameters.AddWithValue("@id", maLeTan);
                cmd.Parameters.AddWithValue("@StartOfMonth", thang);
                cmd.Parameters.AddWithValue("@EndOfMonth", nam);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        luong.Id = Convert.ToInt32(reader["ma_nguoi_dung"]);
                        luong.Ten = reader["ho_ten"].ToString() ?? "";
                        luong.HeSoLuong = Convert.ToSingle(reader["he_so_luong"]);
                        luong.LuongCoBan = Convert.ToSingle(reader["luong_co_ban"]);
                        luong.Thuong = Convert.ToSingle(reader["thuong"]);
                        luong.PhuCap = Convert.ToSingle(reader["phu_cap"]);
                        luong.Phat = Convert.ToSingle(reader["phat"]);
                        luong.SoCa = Convert.ToInt32(reader["so_ca"]);
                    }
                }
            }
            return luong;
