    ...
    internal class VatTuDAO
    {
        ...
        public void ThemThuoc(VatTuDTO thuoc)
        {
            ...
            using (SqlCommand cmd = new SqlCommand("ThemThuoc", dbConnection.Conn))
            {
                ...
                cmd.Parameters.AddWithValue("@ngaySanXuat", DateTime.Parse(thuoc.NgaySanXuat));
                cmd.Parameters.AddWithValue("@ngayHetHan", DateTime.Parse(thuoc.NgayHetHan));
                cmd.Parameters.AddWithValue("@ngayNhap", DateTime.Parse(thuoc.NgayNhap));
                cmd.Parameters.AddWithValue("@gia", thuoc.Gia);
                cmd.Parameters.AddWithValue("@maLoai", 1);
                //MessageBox.Show(thuoc.NgaySanXuat);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.ExecuteNonQuery();
